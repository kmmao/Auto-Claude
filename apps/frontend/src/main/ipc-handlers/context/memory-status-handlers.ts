...
export function buildMemoryStatus(
  ...
  const { available, reason } = checkGraphitiAvailability(projectEnvVars, globalSettings);
  ...
  if (!available) {
    return {
      enabled: true,
      available: false,
      reason: reason || 'Graphiti configuration incomplete'
    };
  }

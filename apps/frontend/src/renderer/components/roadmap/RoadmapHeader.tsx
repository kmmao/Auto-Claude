                ...
                <TooltipContent className="max-w-md">
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {t('roadmap:header.competitorAnalysis.tooltipDesc', {
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="sm" onClick={onAddFeature}>
                ...
                {t('roadmap:header.buttons.addFeature')}
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t('roadmap:header.buttons.addFeatureTooltip')}</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
            ...
            <TooltipContent>{t('roadmap:header.buttons.refreshTooltip')}</TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      {roadmap.targetAudience && (
        <div className="mt-4 flex items-center gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{t('roadmap:header.target.label')}</span>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="text-muted-foreground cursor-help underline decoration-dotted">
                  {t('roadmap:header.target.morePersonas', { count: roadmap.targetAudience.secondary.length })}
                </div>
              </TooltipTrigger>
              <TooltipContent className="max-w-md">
                <div className="space-y-1">
                  <div className="font-semibold mb-2">{t('roadmap:header.target.tooltipTitle')}</div>
      ...
      <div className="mt-4 flex items-center gap-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            ...
            <span className="text-muted-foreground"> {t('roadmap:header.stats.features')}</span>
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm">
            <span className="font-semibold">{roadmap.phases.length}</span>
            <span className="text-muted-foreground"> {t('roadmap:header.stats.phases')}</span>
          </span>
        </div>

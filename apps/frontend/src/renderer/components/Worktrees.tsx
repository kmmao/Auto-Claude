...
export function Worktrees({ projectId }: WorktreesProps) {
  const { t } = useTranslation(['common', 'settings', 'worktrees']);
    ...
    <div className="flex h-full flex-col p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button
          ...
          disabled={isLoading}
        >
          <RefreshCw className={`h-4 w-4 mr-2 ${isLoading ? 'animate-spin' : ''}`} />{t("common:buttons.refresh")}</Button>
      </div>
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-sm">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-medium text-destructive">{t("common:status.error")}</p>
      ...
      {!isLoading && worktrees.length === 0 && (
        <div className="flex h-full flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground">{t('worktrees:noWorktrees.title')}</h3>
          <p className="text-sm text-muted-foreground mt-2 max-w-md">
            {t('worktrees:noWorktrees.description')}
          </p>
        </div>
      )}
                  ...
                  <CardContent className="pt-0">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm mb-4">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span>{t('worktrees:card.filesChanged', { count: worktree.filesChanged })}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <ChevronRight className="h-3.5 w-3.5" />
                        <span>{t('worktrees:card.commitsAhead', { count: worktree.commitCount })}</span>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        ...
                        {t('worktrees:card.merge', { branch: worktree.baseBranch })}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      <Button
                        ...
                        disabled={!task}
                      >
                        <Trash2 className="h-3.5 w-3.5 mr-1.5" />{t("common:buttons.delete")}</Button>
                    </div>
                  </CardContent>
                </Card>
              );
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('worktrees:mergeDialog.title')}
            </DialogTitle>
            <DialogDescription>
              {t('worktrees:mergeDialog.description')}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-4 text-sm space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('worktrees:mergeDialog.sourceBranch')}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('worktrees:mergeDialog.targetBranch')}</span>
                  <span className="font-mono">{selectedWorktree.baseBranch}</span>
                </div>
                <div className="border-t border-border pt-3 mt-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">{t('worktrees:mergeDialog.changes')}</span>
                    <span>
                      {t('worktrees:mergeDialog.changesDetail', { commits: selectedWorktree.commitCount, files: selectedWorktree.filesChanged })}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {mergeResult.conflictFiles && mergeResult.conflictFiles.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-medium">{t('worktrees:mergeDialog.conflictingFiles')}</p>
          ...
          <DialogFooter>
            ...
            >
              {mergeResult ? t('worktrees:mergeDialog.close') : t('worktrees:mergeDialog.cancel')}
            </Button>
            {!mergeResult && (
              <Button
                onClick={handleMerge}
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('worktrees:deleteDialog.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('worktrees:deleteDialog.description')}
              {worktreeToDelete && (
          ...
          <AlertDialogFooter>
            ...
            >
              {isDeleting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  {t('worktrees:deleteDialog.deleting')}
                </>
              ) : (
                <>
                  <Trash2 className="h-4 w-4 mr-2" />{t("common:buttons.delete")}</>
              )}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>

      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            ...
            {t('taskDetail:discardDialog.title')}
          </AlertDialogTitle>
          <AlertDialogDescription asChild>
            <div className="text-sm text-muted-foreground space-y-3">
              <p>
                {t('taskDetail:discardDialog.description')}
              </p>
              <p className="text-destructive">
                {t('taskDetail:discardDialog.warning')}
              </p>
              {worktreeStatus?.exists && (
                <div className="bg-muted/50 rounded-lg p-3 text-sm">
                  <div className="flex justify-between mb-1">
                    <span className="text-muted-foreground">{t('taskDetail:discardDialog.filesChanged')}</span>
                    <span>{worktreeStatus.filesChanged || 0}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t('taskDetail:discardDialog.lines')}</span>
        ...
        <AlertDialogFooter>
          <AlertDialogCancel disabled={isDiscarding}>{t('taskDetail:discardDialog.cancel')}</AlertDialogCancel>
          <AlertDialogAction
            onClick={(e) => {
              e.preventDefault();
              onDiscard();
          ...
          >
            {isDiscarding ? (
              ...
              <>
                ...
                {t('taskDetail:discardDialog.action')}
              </>
            )}
          </AlertDialogAction>
        </AlertDialogFooter>

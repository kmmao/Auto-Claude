...
function getPhaseConfig(
  ...
  // Using hardcoded English strings for short labels here since these are technical terms/brands (Opus, Sonnet, etc.)
  // If we wanted to localize "Low/Med/High", we could return keys instead, but for now assuming these specific technical labels are fine.
  // Actually, we should localize Thinking levels: None, Low, Med, High, Ultra.
  ...
  if (metadata.isAutoProfile && metadata.phaseModels && metadata.phaseThinking) {
    ...
    return {
      ...
      thinking: thinking // Return raw thinking key to be translated in component
    };
  }
  ...
  if (metadata.model && metadata.thinkingLevel) {
    return {
      ...
      thinking: metadata.thinkingLevel // Return raw thinking key
    };
  }
      ...
      <div className="p-4 space-y-2">
        {isLoadingLogs ? (
          ...
          <div className="text-center text-sm text-muted-foreground py-8">
            ...
            <p>{t('taskDetail:logs.empty')}</p>
            <p className="text-xs mt-1">{t('taskDetail:logs.pendingMessage')}</p>
          </div>
        )}
      </div>
    </div>
      ...
      case 'completed':
        return (
          <Badge variant="outline" className="text-xs bg-success/10 text-success border-success/30 flex items-center gap-1">
            ...
            {t('taskDetail:logs.complete')}
          </Badge>
        );
      case 'failed':
        return (
          <Badge variant="outline" className="text-xs bg-destructive/10 text-destructive border-destructive/30 flex items-center gap-1">
            <XCircle className="h-3 w-3" />{t("common:status.failed")}</Badge>
        );
      default:
        return (
          <Badge variant="secondary" className="text-xs text-muted-foreground">{t("common:status.pending")}</Badge>
        );
    }
  };
    ...
    <Collapsible open={isExpanded} onOpenChange={onToggle}>
      <CollapsibleTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-sm">{t(`taskDetail:logs.phases.${phase}`)}</span>
            {hasEntries && (
              <span className="text-xs text-muted-foreground">
                {t('taskDetail:logs.entries', { count: phaseLog?.entries.length })}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            {phaseConfig && (
              <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                ...
                <div className="flex items-center gap-0.5" title={`Thinking: ${t(`taskDetail:logs.thinking.${phaseConfig.thinking}`)}`}>
                  <Brain className="h-3 w-3" />
                  <span>{t(`taskDetail:logs.thinking.${phaseConfig.thinking}`)}</span>
                </div>
              </div>
            )}
            {getStatusBadge()}
      ...
      <CollapsibleContent>
        <div className="mt-1 ml-6 border-l-2 border-border pl-4 py-2 space-y-1">
          {!hasEntries ? (
            <p className="text-xs text-muted-foreground italic">{t('taskDetail:logs.empty')}</p>
          ) : (
...
function LogEntry({ entry }: LogEntryProps) {
  ...
  const getToolInfo = (toolName: string) => {
    switch (toolName) {
      case 'Read':
        return { icon: FileText, label: t('taskDetail:logs.tools.reading'), color: 'text-blue-500 bg-blue-500/10' };
      case 'Glob':
        return { icon: FolderSearch, label: t('taskDetail:logs.tools.searchingFiles'), color: 'text-amber-500 bg-amber-500/10' };
      case 'Grep':
        return { icon: Search, label: t('taskDetail:logs.tools.searchingCode'), color: 'text-green-500 bg-green-500/10' };
      case 'Edit':
        return { icon: Pencil, label: t('taskDetail:logs.tools.editing'), color: 'text-purple-500 bg-purple-500/10' };
      case 'Write':
        return { icon: FileCode, label: t('taskDetail:logs.tools.writing'), color: 'text-cyan-500 bg-cyan-500/10' };
      case 'Bash':
        return { icon: Terminal, label: t('taskDetail:logs.tools.running'), color: 'text-orange-500 bg-orange-500/10' };
      default:
      ...
      <div className="flex flex-col">
        <div className="flex items-center gap-2">
          <div className={cn('inline-flex items-center gap-2 rounded-md px-2 py-1 text-xs', color, 'opacity-60')}>
            ...
            <span className="text-muted-foreground">{t('taskDetail:logs.actions.done')}</span>
          </div>
          {hasDetail && (
            <button
            ...
            >
              {isExpanded ? (
                <>
                  ...
                  <span>{t('taskDetail:logs.actions.hideOutput')}</span>
                </>
              ) : (
                <>
                  <ChevronRight className="h-2.5 w-2.5" />
                  <span>{t('taskDetail:logs.actions.showOutput')}</span>
                </>
              )}
            </button>
          )}
        ...
        {hasDetail && (
          ...
          >
            {isExpanded ? (
              <>
                <ChevronDown className="h-2.5 w-2.5" />
                <span>{t('taskDetail:logs.actions.less')}</span>
              </>
            ) : (
              <>
                <ChevronRight className="h-2.5 w-2.5" />
                <span>{t('taskDetail:logs.actions.more')}</span>
              </>
            )}
          </button>
        )}

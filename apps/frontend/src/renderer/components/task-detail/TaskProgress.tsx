          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">
                {executionPhase ? t(`taskDetail:logs.phases.${executionPhase}`) : EXECUTION_PHASE_LABELS[executionPhase!]}
              </span>
            ...
            {task.executionProgress?.currentSubtask && (
              <p className="text-xs mt-0.5 opacity-70">
                {t('taskDetail:progress.subtaskPrefix')} {task.executionProgress.currentSubtask}
              </p>
            )}
          </div>
        </div>
        ...
        <span className="text-xs text-muted-foreground">
          {hasActiveExecution && task.executionProgress?.message
            ...
            : task.subtasks.length > 0
              ? t('taskDetail:progress.completedSummary', { completed: task.subtasks.filter(c => c.status === 'completed').length, total: task.subtasks.length })
              : t('taskDetail:progress.noSubtasksYet')}
        </span>
        <span className={cn(
      ...
      {hasActiveExecution && (
        <div className="mt-2 flex gap-0.5 h-1.5 rounded-full overflow-hidden bg-muted/30">
          <div
            ...
            style={{ width: '20%' }}
            title={t('taskDetail:progress.phases.tooltip.planning')}
          />
          <div
            className={cn(
              'transition-all duration-300',
            ...
            style={{ width: '60%' }}
            title={t('taskDetail:progress.phases.tooltip.coding')}
          />
          <div
            className={cn(
              'transition-all duration-300',
            ...
            style={{ width: '15%' }}
            title={t('taskDetail:progress.phases.tooltip.review')}
          />
          <div
            className="transition-all duration-300 bg-success/30"
            style={{ width: '5%' }}
            title={t('taskDetail:progress.phases.tooltip.complete')}
          />
        </div>
      )}
    </div>

      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-muted/50 p-3">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              {t('changelog:config.summary', { count: summaryInfo.count, type: summaryInfo.label })}
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('changelog:config.releaseInfo')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="version">{t("common:common.version")}</Label>
              <Input
                id="version"
                value={version}
            ...
            <div className="space-y-2">
              <Label htmlFor="date">{t('changelog:config.date')}</Label>
              <Input
                id="date"
                type="date"
                value={date}
            ...
            {(existingChangelog?.lastVersion || versionBumpDescription) && (
              <div className="text-xs text-muted-foreground space-y-1">
                {existingChangelog?.lastVersion && (
                  <p>{t('changelog:config.previousVersion', { version: existingChangelog.lastVersion })}</p>
                )}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm">{t('changelog:config.outputStyle')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label>{t('changelog:config.format')}</Label>
              <Select
                value={format}
            ...
            <div className="space-y-2">
              <Label>{t('changelog:config.audience')}</Label>
              <Select
                value={audience}
            ...
            <div className="space-y-2">
              <Label>{t('changelog:config.emojis')}</Label>
              <Select
                value={emojiLevel}
          ...
          <CollapsibleContent className="pt-2">
            <Card>
              <CardContent className="pt-4">
                <div className="space-y-2">
                  <Label htmlFor="instructions">{t('changelog:config.instructions')}</Label>
                  <Textarea
                    id="instructions"
                    value={customInstructions}
                    onChange={(e) => onCustomInstructionsChange(e.target.value)}
                    placeholder={t('changelog:config.instructionsPlaceholder')}
                    rows={3}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t('changelog:config.instructionsDesc')}
                  </p>
                </div>
              </CardContent>
            </Card>
